...
export function HeroHeader({
  ...
  return (
    <section className="border-b border-[#314e72] bg-[linear-gradient(180deg,#2c4c71,#25415f)] px-5 py-4 text-white md:px-9">
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.75fr)_260px] lg:items-start">
        <div className="flex min-w-0 flex-col gap-2.5">
          ...
          <h1 className="max-w-none text-[1.18rem] leading-[1.16] font-bold tracking-[0.01em] uppercase md:text-[1.35rem] lg:text-[1.48rem]">
            <span className="text-white">{title} </span>
            <span className="text-[var(--color-accent)]">{highlight}</span>
          </h1>

          <p className="max-w-[62ch] text-justify text-[0.95rem] leading-6 text-white/84">
            {description}
          </p>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {summaryItems.map((item) => (
              <span
                key={item}
                className="rounded-full border border-white/14 bg-white/8 px-3 py-1.5 text-xs font-semibold text-white/82"
              >
                {item}
              </span>
            ))}
        ...
        <aside className="w-full max-w-[260px] rounded-2xl border border-white/10 bg-white/7 px-4 py-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] lg:justify-self-end">
          <h2 className="text-[10px] font-bold uppercase tracking-[0.14em] text-white/68">
            Visão geral
          </h2>
          <div className="mt-3 space-y-2.5 text-justify text-[0.84rem] leading-6 text-white/82">
            <p>{highlights[0]}</p>
            <p>{highlights[1]}</p>
